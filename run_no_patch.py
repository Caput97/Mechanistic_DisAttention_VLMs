...
setproctitle("Attn_VLM_BASELINE_NOPATCH")
...
# 🔴 DIFFERENZA #1: NON importiamo patch_all_attention_layers
# from attn_knockout.patching import patch_all_attention_layers

import attn_knockout
print("USING attn_knockout from:", attn_knockout.__file__)


# 🔴 DIFFERENZA #2: importiamo i tracker "vanilla" (no set/clear mask ranges)
from attn_knockout.tracking_prob_vanilla import track_sentence_prob_layerwise_vanilla
from attn_knockout.tracking_attn_vanilla import track_attention_layerwise_vanilla
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Which assistant query rows are analyzed."
    )
    ...
    parser.add_argument(
        ...
        default="all_regions",
        help=(
            "Which regions to ANALYZE post-hoc (does NOT affect attention). "
    ...
    print(f"✅ [NO PATCH] RUN_MODE={RUN_MODE} | query_scope={query_scope} | video_mode={video_mode} | ✅ mask_modes(analysis)={mask_modes} | attention_mode={attn_mode_str}")
    ...
    output_path_prob = (f"/home/dtesta/Mech_attn_vanilla/results_knockout/"
                        f"BASELINE_NOPATCH_prob_200_16F_{query_scope}_{subdataset}_{video_mode}_regions_{mask_modes_tag}.jsonl")

    output_path_attn = (f"/home/dtesta/Mech_attn_vanilla/results_knockout/"
                        f"BASELINE_NOPATCH_attn_200_16F_{query_scope}_{subdataset}_{video_mode}_regions_{mask_modes_tag}_{attn_mode_str}.jsonl")

    output_path_attn_per_head = (
        f"/home/dtesta/Mech_attn_vanilla/results_knockout/"
        f"BASELINE_NOPATCH_attn_per_head_200_16F_{query_scope}_{subdataset}_{video_mode}_regions_{mask_modes_tag}_{attn_mode_str}.jsonl"
    )

    attn_pt_dir = (f"/home/dtesta/Mech_attn_vanilla/results_knockout/"
                   f"BASELINE_NOPATCH_attn_matrices_pt_{query_scope}_{subdataset}_{video_mode}_regions_{mask_modes_tag}_{attn_mode_str}")
    ...
    # 🔴 DIFFERENZA #3: NON patchiamo nulla
    # patch_all_attention_layers(model)

    print(f"Starting tracking [NO PATCH] (mode={RUN_MODE})...\n")

    failed_videos = []

    for idx, item in enumerate(tqdm(data[:200], desc="Processing items")):
        try:
            ...
            foil = item["foil"]
            target = item["caption_position"]  # 0 or 1

            if RUN_MODE == "prob":
                final_results = {"caption": {}, "foil": {}}

                for mode in mask_modes:
                    prob_track = track_sentence_prob_layerwise_vanilla(
                        prompt,
                        video_path,
                        caption,
                        foil,
                        ...
                        tokenizer,
                        lm_head,
                        analysis_mode=mode,       # 👈 solo per etichettare / confronto
                ...
                for mode in mask_modes:
                    attn_track = track_attention_layerwise_vanilla(
                        prompt,
                        video_path,
                        caption,
                        foil,
                        ...
                        processor,
                        tokenizer,
                        analysis_mode=mode,        # 👈 solo per region-focus post-hoc
                        query_scope=query_scope,
